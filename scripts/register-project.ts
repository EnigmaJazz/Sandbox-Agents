...
const PROFILE_PATH = join(
  ...
  "nono",
  "profile",
  "opencode-secure.json",
);
const LAUNCHER_PATH = join(
  import.meta.dir,
  "..",
  "scripts",
  "secure-launcher.conf",
);
...
const TARGETS = [
  ...new Set(process.argv.slice(2).filter((a) => !FLAGS.has(a))),
];
...
interface Profile {
  filesystem?: { read?: string[]; allow_file?: string[]; allow?: string[] };
}
...
function existingBrokerEnvEntries(): { id: string; path: string }[] {
  ...
  try {
    const match = readFileSync(BROKER_ENV_PATH, "utf8").match(
      /^BROKER_PROJECTS=(.*)$/m,
    );
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        if (typeof entry?.id === "string")
          entries.push({ id: entry.id, path: entry.path });
      }
    }
  } catch {
...
function updateBrokerEnv(path: string, id: string): LocationStatus | string {
  ...
  if (!existsSync(BROKER_ENV_PATH))
    return "skipped (broker.env missing; not created)";
  ...
  try {
    const parsed = JSON.parse(body);
    if (!Array.isArray(parsed))
      return "skipped (BROKER_PROJECTS is not a JSON array)";
    projects = parsed;
  } catch {
  ...
  if (!DRY_RUN) {
    ...
    const line =
      quote === null
        ? `BROKER_PROJECTS=${json}`
        : `BROKER_PROJECTS=${quote}${json}${quote}`;
...
function setupGit(
  ...
  if (hasName.ok) {
    ...
    const setName = execOk(
      "git",
      ["config", "--local", "user.name", user.login],
      project,
    );
  ...
  if (hasOrigin.ok) {
    ...
    if (!repoExists.ok) {
      ...
      const add = execOk(
        "git",
        ["remote", "add", "origin", `git@github.com:${user.login}/${base}.git`],
        project,
      );
  ...
  if (!authed.ok) {
    ...
    plan.ghError =
      "--create-remote requires gh authentication (run gh auth login)";
    return plan;
  }
  if (user === null) {
    plan.ghRemote = "skipped";
  ...
  if (hasHead.ok) {
    ...
    if (!pushed.ok)
      plan.warnings.push(
        `git push -u origin main failed for ${base} (remote was created)`,
      );
  }
  return plan;
}
